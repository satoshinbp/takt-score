        ...
        >
          {scores.map((s) => (
            <div
              key={s.id}
              onClick={() => onSelect(s)}
              className="rounded-md p-4 cursor-pointer transition-all duration-[140ms] relative overflow-hidden bg-[var(--s1)] border border-[var(--bd)] hover:border-[var(--bd2)] hover:bg-[var(--s2)] hover:-translate-y-px hover:shadow-[0_4px_20px_rgba(0,0,0,.4)]"
            >
...
function NewScoreCard({ onClick }: { onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="rounded-md p-4 cursor-pointer transition-all duration-[140ms] flex flex-col items-center justify-center gap-1.5 min-h-[130px] border border-dashed border-[var(--bd)] text-[var(--tm)] hover:text-[var(--t)] hover:border-[var(--acc)] hover:bg-[var(--acc-d)]"
    >
      <Plus size={22} />
